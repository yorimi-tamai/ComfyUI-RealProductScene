...
一張去背產品 PNG（左）→ 自動生成場景並把**真實產品**合成進去（右）。產品像素 100%
來自原圖——外觀、Logo、文字、比例完整保留，AI 不重畫產品，只負責它周圍的場景、接地與光影。

| 輸入：去背產品 PNG | 輸出：合成產品情境圖 |
|---|---|
| <img src="docs/example-product.png" width="320" alt="去背產品 PNG（透明背景）"> | <img src="docs/example-scene.png" width="320" alt="合成後的產品情境圖"> |
...
pip install -r ComfyUI-RealProductScene/requirements.txt   # Pillow（ComfyUI 通常已內建）
# 重啟 ComfyUI
```
