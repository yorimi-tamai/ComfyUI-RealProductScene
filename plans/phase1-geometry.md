...
- [x] 3. 從 `product_scene_composite_v2_api.json` 拆出 `workflows/comfyui_api/bg_generate_api.json`（只背景生成 + 輸出）
- [x] 4. 拆出 `workflows/comfyui_api/composite_api.json`（LoadImage 背景 + 產品 + 陰影鏈 72–79 + 產品合成 69 + Save）
- [x] 5. `scripts/comfy_client.py`：POST /prompt、poll /history/{id}、GET /view 取圖
...
- [x] 9. `scripts/generate.py`：主編排器（讀 config → 驗證 PNG alpha，JPG/無 alpha 報明確錯 → 背景 graph → 幾何 → 合成 graph → 存最終圖）；CLI `python scripts/generate.py`
- [x] 10. 端到端驗證（見驗收條件）
